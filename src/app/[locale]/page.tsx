    ...
    >
      <div className="grid gap-14 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
        ...
        <RevealSection delay={0.08}>
          <BlueprintSystemMap content={content} />
        </RevealSection>
      </div>
    </section>
  );
...
function BlueprintSystemMap({ content }: { content: ServicesContent }) {
        ...
        <div className="relative z-10 flex h-full flex-col justify-between">
          ...
          <div className="rounded-[1.2rem] border border-white/10 bg-black/55 p-4 backdrop-blur">
            <p className="text-[10px] uppercase tracking-[0.2em] text-cyan-300/70">
              {content.premiumBuildLabel}
            </p>
            <p className="mt-3 text-sm leading-7 text-white/68">
              {content.premiumBuildText}
            </p>
          </div>
        </div>
      </div>
