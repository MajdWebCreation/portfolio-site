...
type AdminPageHeaderProps = {
  label?: string;
  title: string;
  /** Status badges or a key figure, on one row under the title. */
  meta?: ReactNode;
...
/** Opening of an admin page: eyebrow, h1, optional status row, one context line, optional actions. */
  ...
  return (
    <header className="flex flex-wrap items-end justify-between gap-x-8 gap-y-4 pb-2">
      <div className="min-w-0">
        ...
        {meta ? <div className="mt-2.5 flex flex-wrap items-center gap-x-3 gap-y-2">{meta}</div> : null}
