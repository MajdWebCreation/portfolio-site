...
/**
 * Sessions per day, this period over the one before it, as one small
...
function path(points: DailyPoint[], max: number): string {
  ...
  return points
    .map((point, index) => {
      ...
      const y = H - PAD - (max > 0 ? (point.sessions / max) * (H - PAD * 2) : 0);
      return `${index === 0 ? "M" : "L"}${x.toFixed(1)},${y.toFixed(1)}`;
    })
    .join(" ");
}

export default function TrendLine({ current, previous, label }: { current: DailyPoint[]; previous: DailyPoint[]; label: string }) {
  const max = Math.max(1, ...current.map((p) => p.sessions), ...previous.map((p) => p.sessions));
  const total = current.reduce((sum, p) => sum + p.sessions, 0);

  return (
    <figure className="m-0">
      <svg viewBox={`0 0 ${W} ${H}`} role="img" aria-label={`${label}: ${total} sessies in deze periode, per dag`} className="block h-auto w-full max-w-full">
      ...
      <figcaption className="mt-2 flex flex-wrap gap-x-5 text-[0.8rem] text-muted">
        ...
        <span>Piek {max} sessies op een dag</span>
      </figcaption>
    </figure>
  );
}
