...
/*
  ...
  As of this implementation the general terms (AV v1.0, /nl/algemene-
  voorwaarden) do not provide for a reminder fee. So the amount below is
  *copy*, and copy only. It appears in one sentence of the second reminder, as
  a warning about what a next step could cost -- and nowhere else:

    - it is never added to an invoice, its lines or its total;
    ...
    - the payment button collects the invoice's own outstanding amount and
      nothing more;
    - it is not described as statutory collection costs, because it is not.

  Nothing in the database can hold it: there is no column for it anywhere in
  the collection tables. That is on purpose -- the guarantee is structural,
  ...
  after the terms are updated, change `reminderFeeCents`. Both are read in
  exactly one place: the second reminder's body.
*/
export const reminderFeeAnnounced = true;

/** EUR 20,00. Announced as a possibility on day 7; never charged by this code. */
