 ...
 *
 * `--content-only` imports an editorially rewritten set of the same twenty
 * articles: the prose changed, nothing else did. The source files then carry
 * their internal links inline, so the `links` and `serviceLinks` tables are
...
function rewrite(slug, body) {
  ...
  */
  ...
  /*
    A rewritten article carries its links in the prose already, so there is
    nothing to find and replace. `requiredLinks` below checks that the same
    set of links survived the rewrite.
  */
  if (contentOnly) return text;
...
/*
  The hrefs the `links` and `serviceLinks` tables add to an article. A rewrite
  writes them into the prose itself, so this is what it has to keep: losing an
  internal link while reworking a paragraph is the easy mistake to make.
*/
function requiredLinks(slug) {
  const hrefs = new Set();
  for (const [, replaceWith] of [...(links[slug] ?? []), ...(serviceLinks[slug] ?? [])]) {
    for (const match of replaceWith.matchAll(/\[[^\]]+\]\((\/[^)]+)\)/g)) hrefs.add(match[1]);
  }
  return hrefs;
}

function checkRequiredLinks(slug, blocks) {
    ...
    const texts =
      block.type === "list"
        ...
        : block.type === "table"
          ...
          : [block.content];
    for (const text of texts) {
      for (const match of text.matchAll(/\[[^\]]+\]\((\/[^)]+)\)/g)) present.add(match[1]);
    }
  }

  const missing = [...requiredLinks(slug)].filter((href) => !present.has(href));
  if (missing.length) throw new Error(`${slug}: rewrite dropped internal link(s) ${missing.join(", ")}`);
}
...
/*
  The content-only migration. The twenty rows already exist and everything
  about them -- slug, title, excerpt, category, publication date, SEO fields,
  cover -- stays exactly as it was applied. Only the article body changes, so
  this updates one column and asserts up front that all twenty rows are there
  to update.
*/
const contentHeader = `-- The twenty articles, editorially rewritten.
--
-- Generated by scripts/import-articles.mjs --content-only. The library was
-- seeded by 20260911094500_articles_library_seed.sql and its metadata was
-- polished by 20260911100500_article_metadata_polish.sql; both are applied
-- and stay untouched.
--
-- This migration changes the prose and nothing else. Every article was
-- rewritten as Dutch long-form copy -- fewer bullet lists, no pseudo-code,
-- paragraphs that argue instead of enumerate -- while keeping the same
-- headings' subject matter, the same internal links, the same sources and
-- the same conclusions. Slug, title, excerpt, category, publication date,
-- SEO fields and cover image are deliberately not in the statement below, so
-- the publication plan and the metadata polish survive as applied.
...
${rows.map((row) => `    ${quote(row.slug)}`).join(",\n")}
  ...
  if found <> ${rows.length} then
    raise exception 'expected ${rows.length} seeded articles to rewrite, found %', found;
  end if;
end
$$;
...
const contentValues = rows
  .map((row) => `  (${quote(row.slug)},
   ${quote(row.content)}::jsonb)`)
  .join(",\n");
